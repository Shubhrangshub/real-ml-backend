...
export default function DeployView() {
  ...
  const [confirmDeploy, setConfirmDeploy] = useState(null); // model pending confirmation
              ...
              {undeployed.map(model => (
                <div key={model.modelId} className="flex items-center justify-between p-3 rounded-xl border hover:bg-accent/20 transition-colors" data-testid={`deploy-model-${model.modelId}`}>
                  ...
                  <Button size="sm" className="gap-1.5 bg-gradient-to-r from-violet-500 to-fuchsia-500 hover:from-violet-600 hover:to-fuchsia-600 text-white"
                    onClick={() => setConfirmDeploy(model)} disabled={deployingId === model.modelId} data-testid={`deploy-btn-${model.modelId}`}>
