...
import { AlertCircle, Mail, Lock, User, Zap } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-zinc-950 dark:to-zinc-900 p-4" data-testid="auth-page">
      <Card className="w-full max-w-md shadow-xl border-border/60" data-testid="auth-card">
        <CardHeader className="text-center pb-2">
          ...
          <CardDescription>{mode === 'login' ? 'Sign in to your account' : 'Create your account'}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <p className="text-center text-sm text-muted-foreground">
            ...
            <button onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError(''); }} className="text-violet-600 dark:text-violet-400 font-medium hover:underline" data-testid="auth-toggle-mode">
