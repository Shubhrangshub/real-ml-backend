    ...
    <div className="min-h-screen bg-background" data-testid="app-root">
      ...
      <div className="pl-64">
        <motion.header initial={{ y: -100 }} animate={{ y: 0 }} className="sticky top-0 z-30 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="flex items-center justify-between px-4 lg:px-6 py-2.5 gap-2 lg:gap-4">
            ...
            <div className="flex items-center gap-1 lg:gap-1.5 shrink-0 overflow-x-auto max-w-[60%] lg:max-w-none">
              <Button variant="ghost" size="sm" onClick={() => { try { localStorage.removeItem('automl_tour_seen'); localStorage.removeItem('automl_milestones_dismissed'); } catch {} window.location.reload(); }} data-testid="guide-toggle-btn" className="h-8 px-2 lg:px-2.5 text-xs" title="Guide"><BookOpen className="h-3.5 w-3.5 lg:mr-1.5" /><span className="hidden lg:inline">Guide</span></Button>
              {(trainingResult || unsupervisedResult) && !viewOnlyMode && <Button variant="ghost" size="sm" onClick={() => handleSaveAnalysis()} data-testid="save-analysis-btn" className="h-8 px-2 lg:px-2.5 text-xs" title="Save"><Save className="h-3.5 w-3.5 lg:mr-1.5" /><span className="hidden lg:inline">Save</span></Button>}
              {(trainingResult || unsupervisedResult) && !viewOnlyMode && <Button variant="ghost" size="sm" onClick={handleShareAnalysis} disabled={exportLoading === 'share'} data-testid="share-analysis-btn" className="h-8 px-2 lg:px-2.5 text-xs" title="Share">{exportLoading === 'share' ? <><span className="h-3.5 w-3.5 lg:mr-1.5 animate-spin rounded-full border-2 border-current border-t-transparent" />...</> : <><Share2 className="h-3.5 w-3.5 lg:mr-1.5" /><span className="hidden lg:inline">Share</span></>}</Button>}
              {(trainingResult || shapGlobal || limeResult || predictionHistory?.length > 0) && !viewOnlyMode && <>
                <Button variant="ghost" size="sm" onClick={handleExportPDF} disabled={!!exportLoading} data-testid="export-pdf-btn" title="Download PDF Report" className="h-8 px-2 lg:px-2.5 text-xs font-semibold text-violet-600 dark:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-950/30">{exportLoading === 'pdf' ? <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-current border-t-transparent" /> : <><FileDown className="h-3.5 w-3.5 lg:mr-1.5" /><span className="hidden lg:inline">Report</span></>}</Button>
                <Button variant="ghost" size="sm" onClick={handleExportSheets} disabled={!!exportLoading} data-testid="export-sheets-btn" title="Export to Google Sheets" className="h-8 px-2 lg:px-2.5 text-xs">{exportLoading === 'sheets' ? <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-current border-t-transparent" /> : <><Sheet className="h-3.5 w-3.5 lg:mr-1.5" /><span className="hidden lg:inline">Sheets</span></>}</Button>
                <Button variant="ghost" size="sm" onClick={handleExportCSV} disabled={!!exportLoading} data-testid="export-csv-btn" title="Download CSV" className="h-8 px-2 lg:px-2.5 text-xs">{exportLoading === 'csv' ? <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-current border-t-transparent" /> : <><Download className="h-3.5 w-3.5 lg:mr-1.5" /><span className="hidden lg:inline">CSV</span></>}</Button>
              </>}
              {csvText && !viewOnlyMode && <Button variant="ghost" size="sm" onClick={handleClearSession} data-testid="clear-session-btn" className="h-8 px-2 lg:px-2.5 text-xs text-destructive hover:text-destructive hover:bg-destructive/10" title="Clear"><Trash2 className="h-3.5 w-3.5 lg:mr-1.5" /><span className="hidden lg:inline">Clear</span></Button>}
